...
class ExampleTests {
    ...
    void filter() {
        ...
        assertEquals("[2, 4]",
                $.just(1, 2, 3, 4, 5).filter(i -> i % 2 == 0).value().toString());
    }
    ...
    void flatMap() {
        ...
        assertEquals("[]", $.just(1, 2, 3).flatMap(i -> null).value().toString());
    }
    ...
    void orderBy() {
        assertEquals("[C1, A2, B3]",
                $.just("C1", "A2", "B3").orderBy(it -> Integer.parseInt(it.substring(1)), Direction.ASC).value().toString());
        assertEquals("[A2, B3, C1]",
                $.just("C1", "A2", "B3").orderBy(Function.identity(), Direction.ASC).value().toString());
    }
